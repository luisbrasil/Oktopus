...
class UsuarioDAOSQLite implements UsuarioInterfaceDao {
  ...
  Future<List<Usuario>> consultarTodos() async {
    Database db = await Conexao.criar();
    List<Usuario> lista = (await db.query('usuario'))
        .map<Usuario>(converterUsuario)
        .toList();
    return lista;
  }
  ...
  Future<Usuario> salvar(Usuario usuario) async {
    ...
    String sql;
    if (usuario.id == null) {
      sql = 'INSERT INTO usuario (nome, cpf, email, urlAvatar) VALUES (?,?,?,?)';
      ...
      sql =
          'UPDATE usuario SET nome = ?, cpf = ?, email = ?, urlAvatar = ? WHERE id = ?';
      db.rawUpdate(sql, [
        usuario.nome,
        usuario.cpf,
        usuario.email,
